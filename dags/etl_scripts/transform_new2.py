...
def transform_data(neonatal_csv_path, skilled_health_csv_path, antenatal_csv_path, maternal_mortality_csv_path, target_dir, to_parquet=False, to_csv=True):
    ...
    neonatal_dim_df = neonatal_dim_df.rename(columns={'Indicator': 'NMR_Indicator', 
                                                      'Year': 'Year_Id', 
                                                      'Country': 'Country_Id', 
                                                      'WHO region': 'WHO_Region_Id', 
                                                      'World bank income group':'WB_Income_Group_Id', 
                                                      'Value Numeric':'NMR_Value', 
                                                      'Value low':'NMR_Value_Low', 
                                                      'Value high': 'NMR_Value_High'})

    neonatal_dim_df['NMR_Id'] = neonatal_dim_df.index

    neonatal_dim_df = neonatal_dim_df[['NMR_Id', 'NMR_Indicator', 
                                       'Year_Id', 'Country_Id', 'WHO_Region_Id', 'WB_Income_Group_Id', 
                                       'NMR_Value', 'NMR_Value_Low', 'NMR_Value_High']]

    # -------------------------------------------------------------------------------------#
    ## ANTENATAL TABLE
    antenatal_dim_df = antenatal_df[['Indicator', 'Year', 'Country', 'WHO region', 'World bank income group', 'Value Numeric']]
    antenatal_dim_df = antenatal_dim_df.rename(columns={'Indicator':'ACC_Indicator',
                                                        'Year': 'Year_Id',
                                                        'Country': 'Country_Id', 
                                                        'WHO region': 'WHO_Region_Id', 
                                                        'World bank income group':'WB_Income_Group_Id', 
                                                        'Value Numeric':'ACC_Value'})

    antenatal_dim_df['ACC_Id'] = antenatal_dim_df.index

    antenatal_dim_df = antenatal_dim_df[['ACC_Id', 'ACC_Indicator', 
                                         'Year_Id', 'Country_Id', 'WHO_Region_Id', 'WB_Income_Group_Id',
                                         'ACC_Value']]
    ...
    maternal_mortality_dim_df = maternal_mortality_dim_df.rename(columns={'Indicator':'MMR_Indicator',
                                                                          'Year': 'Year_Id', 
                                                                          'Country': 'Country_Id', 
                                                                          'WHO region': 'WHO_Region_Id', 
                                                                          'World bank income group':'WB_Income_Group_Id', 
                                                                          'Value Numeric':'MMR_Value', 
                                                                          'Value low': 'MMR_Value_Low', 
                                                                          'Value high': 'MMR_Value_High'})

    maternal_mortality_dim_df['MMR_Id'] = maternal_mortality_dim_df.index

    maternal_mortality_dim_df = maternal_mortality_dim_df[['MMR_Id', 'MMR_Indicator', 
                                                           'Year_Id', 'Country_Id', 'WHO_Region_Id', 'WB_Income_Group_Id', 
                                                           'MMR_Value', 'MMR_Value_Low', 'MMR_Value_High']]
    ...
    skilled_health_dim_df = skilled_health_dim_df.rename(columns={'Indicator': 'SHP_Indicator',
                                                                  'Year': 'Year_Id', 
                                                                  'Country': 'Country_Id', 
                                                                  'WHO region': 'WHO_Region_Id', 
                                                                  'World bank income group':'WB_Income_Group_Id', 
                                                                  'Value Numeric': 'SHP_Value'})

    skilled_health_dim_df['SHP_Id'] = skilled_health_dim_df.index

    skilled_health_dim_df = skilled_health_dim_df[['SHP_Id', 'SHP_Indicator', 
                                                   'Year_Id', 'Country_Id', 'WHO_Region_Id', 'WB_Income_Group_Id', 
                                                   'SHP_Value']]

    # -------------------------------------------------------------------------------------#
    ## Merge the dim tables for alignment
    merged_df1 = pd.merge(neonatal_dim_df, antenatal_dim_df, how='outer',
                          left_on = ['Year_Id', 'Country_Id', 'WHO_Region_Id', 'WB_Income_Group_Id'],
                          right_on = ['Year_Id', 'Country_Id', 'WHO_Region_Id', 'WB_Income_Group_Id'])

    merged_df2 = pd.merge(maternal_mortality_dim_df, skilled_health_dim_df, how='outer',
                          left_on = ['Year_Id', 'Country_Id', 'WHO_Region_Id', 'WB_Income_Group_Id'],
                          right_on = ['Year_Id', 'Country_Id', 'WHO_Region_Id', 'WB_Income_Group_Id'])

    full_merged_df = pd.merge(merged_df1, merged_df2, how='outer',
                          left_on = ['Year_Id', 'Country_Id', 'WHO_Region_Id', 'WB_Income_Group_Id'],
                          right_on = ['Year_Id', 'Country_Id', 'WHO_Region_Id', 'WB_Income_Group_Id'])

    # -------------------------------------------------------------------------------------#
    ## Change merged table columns into the IDS for other dim tables
    ## year, country, WHO region, and WB income group

    ## create the mappings
    year_map = pd.Series(year_dim_df.Year_Id.values, index=year_dim_df.Year).to_dict()
    country_map = pd.Series(country_dim_df.Country_Id.values, index=country_dim_df.Country).to_dict()
    who_region_map = pd.Series(region_dim_df.WHO_Region_Id.values, index=region_dim_df.WHO_Region).to_dict()
    wb_income_map = pd.Series(income_dim_df.WB_Income_Group_Id.values, index=income_dim_df.WB_Income_Group).to_dict()
    
    ##use the mappings to replace the values with their IDs
    full_merged_df.replace({'Year_Id': year_map,
                            'Country_Id': country_map,
                            'WHO_Region_Id': who_region_map,
                            'WB_Income_Group_Id':wb_income_map
                            },
                            inplace=True)

    neonatal_dim_df.replace({'Year_Id': year_map,
                            'Country_Id': country_map,
                            'WHO_Region_Id': who_region_map,
                            'WB_Income_Group_Id':wb_income_map
                            },
    ...
    antenatal_dim_df.replace({'Year_Id': year_map,
                            'Country_Id': country_map,
                            'WHO_Region_Id': who_region_map,
                            'WB_Income_Group_Id':wb_income_map
                            },
    ...
    maternal_mortality_dim_df.replace({'Year_Id': year_map,
                            'Country_Id': country_map,
                            'WHO_Region_Id': who_region_map,
                            'WB_Income_Group_Id':wb_income_map
                            },
    ...
    skilled_health_dim_df.replace({'Year_Id': year_map,
                            'Country_Id': country_map,
                            'WHO_Region_Id': who_region_map,
                            'WB_Income_Group_Id':wb_income_map
                            },
    ...
    healthcare_disparities_fct_df = full_merged_df[['Year_Id', 'Country_Id', 'WHO_Region_Id', 'WB_Income_Group_Id',
                                                    'SHP_Id', 'MMR_Id', 'NMR_Id', 'ACC_Id']]
